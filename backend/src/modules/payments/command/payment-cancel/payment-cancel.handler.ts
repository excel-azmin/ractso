...
import { PaymentCancelStatusCommand } from './payment-cancel.command';

@CommandHandler(PaymentCancelStatusCommand)
export class PaymentCancelStatusHandler
  implements ICommandHandler<PaymentCancelStatusCommand>
{
  constructor(private readonly prismaService: PrismaService) {}
  async execute(command: PaymentCancelStatusCommand) {
    ...
    const result = await this.prismaService.$transaction(async (prisma) => {
      const updatedPayment = await prisma.payments.update({
        where: { tran_id: paymentInfo.tran_id },
        data: { status: 'CANCELLED', paymentType: paymentInfo.card_brand },
      });
